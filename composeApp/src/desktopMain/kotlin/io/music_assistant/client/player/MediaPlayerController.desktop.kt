...
actual class MediaPlayerController actual constructor(
    ...
    actual fun getCurrentPosition(): Long? = mediaPlayer?.status()?.time()

    actual fun getDuration(): Long? = mediaPlayer?.media()?.info()?.duration()

    actual fun stop() {
        mediaPlayer?.controls()?.stop()
    }

    actual fun isPlaying(): Boolean = mediaPlayer?.status()?.isPlaying ?: false
